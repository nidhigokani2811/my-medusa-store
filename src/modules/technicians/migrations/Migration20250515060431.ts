...
export class Migration20250515060431 extends Migration {

  override async up(): Promise<void> {
    this.addSql(`alter table if exists "technician" drop constraint if exists "technician_payload_technician_id_unique";`);
    ...
    this.addSql(`create table if not exists "technician" ("id" text not null, "name" text not null, "email" text not null, "payload_technician_id" text not null, "nylas_grant_id" text null, "nylas_calendar_id" text null, "nylas_configuration_id" text null, "payload_tenant_id" text not null, "tenant_id" text not null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "technician_pkey" primary key ("id"));`);
    this.addSql(`CREATE UNIQUE INDEX IF NOT EXISTS "IDX_technician_email_unique" ON "technician" (email) WHERE deleted_at IS NULL;`);
    this.addSql(`CREATE UNIQUE INDEX IF NOT EXISTS "IDX_technician_payload_technician_id_unique" ON "technician" (payload_technician_id) WHERE deleted_at IS NULL;`);
